...
let givePoint = async function (req, res, next) {
    try {
        ...
        if (point % 25 !== 0 && point > 250)
            throw new AppError(400, 'wrong input for point');
        let tenant = await Tenant.Repository.fetchOne(tenant_name);
        let user = await User.Repository.fetchOne(user_id);
        if (!user || !tenant)
        ...
        user.addPoint(point);
        let pt = PointTransaction.Repository.create(tenant, user, point);
...
let redeemItem = async function (req, res, next) {
    try {
        ...
        let item = await Item.Repository.fetchOne(item_id);
        let user = await User.Repository.fetchOne(user_id);
        if (!user || !item)
        ...
        let it = ItemTransaction.Repository.create(item, user, point, quantity);
